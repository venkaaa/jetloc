...
if ( function_exists('get_the_image') ) { 
	...
	echo ( $the_thumb_html_as_array[src] ) ? '<div class="small-custom-frame-wrapper alignleft'.$shadow_class.'"><div class="custom-frame-inner-wrapper"><div class="custom-frame-padding"><a href="'.get_permalink().'" title="'.the_title('', '', false).'"><img src="'.udesign_process_image( $the_thumb_html_as_array[url], $post_thumb_width, $post_thumb_height, 1, 100 ).'" width="'.$post_thumb_width.'" height="'.$post_thumb_height.'" alt="'.$the_thumb_html_as_array[alt].'" /></a></div></div></div>' : '';
	...
	if ( $udesign_options['default_thumb_on'] == 'yes' ) { // Default thumbnail option is selected
	    if ( $the_post_thumbnail_link ) { // look for the thumbnail passed as a 'post_thumbnail' or 'thumbnail' custom field ?>
                    <div class="small-custom-frame-wrapper alignleft<?php echo $shadow_class; ?>"><div class="custom-frame-inner-wrapper"><div class="custom-frame-padding"><a href="<?php the_permalink() ?>" title="<?php the_title(); ?>"><img src="<?php echo udesign_process_image( $the_post_thumbnail_link, $post_thumb_width, $post_thumb_height, 1, 100 ); ?>" width="<?php echo $post_thumb_width; ?>" height="<?php echo $post_thumb_height; ?>" alt="<?php the_title(); ?>" /></a></div></div></div>
