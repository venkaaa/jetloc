...
function udesign_init_styles() {
	...
	wp_enqueue_style('u-design-superfish_menu', get_bloginfo('template_url') . '/scripts/superfish-1.4.8/css/superfish.css', false, '1.0', 'screen');
	...
	wp_enqueue_style('u-design-style', get_bloginfo('template_url') . "/styles/style1/css/style.css", false, '2.0.0', 'screen');
        ...
        if ( get_theme_mod( 'udesign_custom_styles_use_css_file' ) ) { // load "custom_style.css" file
            wp_enqueue_style('u-design-custom-style', get_bloginfo('template_url') . '/styles/custom/custom_style.css', false, '', 'screen');
        } else { // otherwise use "custom_style.php" file
            wp_enqueue_style('u-design-custom-style', get_bloginfo('template_url') . '/styles/custom/custom_style.php', false, '', 'screen');
        }
        
        if ( $udesign_options['enable_responsive'] ) {
            wp_enqueue_style('u-design-responsive', get_bloginfo('template_url') . '/styles/common-css/responsive.css', false, '1.0', 'screen');
        }
        
        if ( $udesign_options['enable_default_style_css'] ) {
            wp_enqueue_style('u-design-style-orig', get_stylesheet_directory_uri() . "/style.css", false, '1.0', 'screen');
        }
    }
}
...
function udesign_init_scripts() {
	...
	if( is_front_page() && $current_slider == '4' ) {
	    wp_enqueue_script('cycle', get_bloginfo('template_url')."/sliders/cycle/jquery.cycle.all.min.js", array('jquery'), '2.99', true);
	    wp_enqueue_script('cycle1', get_bloginfo('template_url')."/sliders/cycle/cycle1/cycle1_script.js", array('jquery'), '1.0.0', true);
	}

	// Cycle 2 Slider Plugin
	if( is_front_page() && $current_slider == '5' ) {
	    wp_enqueue_script('cycle', get_bloginfo('template_url')."/sliders/cycle/jquery.cycle.all.min.js", array('jquery'), '2.99', true);
	    wp_enqueue_script('cycle2', get_bloginfo('template_url')."/sliders/cycle/cycle2/cycle2_script.js", array('jquery'), '1.0.0', true);
	}
	...
	if( is_front_page() && $current_slider == '6' ) {
	    ...
	    wp_enqueue_script('cycle3', get_bloginfo('template_url')."/sliders/cycle/cycle3/cycle3_script.js", array('jquery'), '1.0.0', true);
	}

	// PrettyPhoto script
	if( $udesign_options['enable_prettyPhoto_script'] && !WP_PRETTY_PHOTO_PLUGIN_ACTIVE ) {
            wp_enqueue_script('pretty_photo_lib', get_bloginfo('template_url')."/scripts/prettyPhoto/js/jquery.prettyPhoto.js", array('jquery'), '3.1.5', true);
	...
	// jQuery validation script
        if ( is_page_template('page-Contact.php') ) {
            wp_enqueue_script('jquery_validate_lib', get_bloginfo('template_url')."/scripts/jquery-validate/jquery.validate.min.js", array('jquery'), '1.8.1', true);
            wp_enqueue_script('masked_input_plugin', get_bloginfo('template_url')."/scripts/masked-input-plugin/jquery.maskedinput.min.js", array('jquery'), '1.3', true);
        }
	...
	wp_enqueue_script('superfish-menu', get_bloginfo('template_url')."/scripts/superfish-1.4.8/js/superfish.combined.js", array('jquery'), '1.0.0', true);
        
	// Miscellaneous JS scripts
	wp_enqueue_script('scripts', get_bloginfo('template_url')."/scripts/script.js", array('jquery'), '1.0', true);
...
<?php                       else : ?>
                                <h1 class="pagetitle"><?php printf( __('Archive for the <em>%s</em> Category', 'udesign' ), single_cat_title("", false) ); ?></h1>
<?php                       endif; ?>
...
<?php			elseif( is_tag() ) : /* If this is a tag archive */ ?>
                            <h1 class="pagetitle"><?php printf( __('Posts Tagged <em>%s</em>', 'udesign' ), single_tag_title("", false) ); ?></h1>
...
// Add 'u-design-responsive-on' to the body classes
if ( $udesign_options['enable_responsive'] ) {
    // Add specific CSS class by filter
    function udesign_body_class_names($classes) {
            global $udesign_options;
            // add 'u-design-responsive-on' to the $classes array
            $classes[] = 'u-design-responsive-on';
            // return the $classes array
            return $classes;
    }
    add_filter('body_class','udesign_body_class_names');
}
