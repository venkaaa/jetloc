...
(function($){
    ...
    $(function(){
    		...
    		if(t.length){
    			...
    			if(t_options.length){
    				...
    				if(t_options.length){
    					...
    					t_parent.children('.tesla-option-container').find('input, textarea, select[name]').prop('disabled',false);
		...
		$('#slide_options').on('change','select:not([name])',function(){
			...
			t_recursive($(this).parent('.tesla-option'));
		...
		//editor type
		tinymce.init({
			selector: '.tesla-input-editor:not([disabled])',
			plugins: 'link image media charmap paste preview hr searchreplace visualblocks visualchars code autolink advlist autoresize contextmenu directionality lists textcolor fullscreen',
			toolbar: 'undo redo | formatselect | bold italic | alignleft aligncenter alignright alignjustify  | bullist numlist outdent indent | link image | fontselect fontsizeselect | forecolor backcolor | ltr rtl | fullscreen',
			autoresize_max_height: '500px',
			autoresize_min_height: '200px',
			fontsize_formats: '10px 12px 14px 18px 24px 30px 36px 48px 60px 72px'
		});
