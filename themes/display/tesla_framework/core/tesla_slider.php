...
class Tesla_slider{
	...
	public static function slider_admin_enqueue($hook){
		...
		if('post-new.php'===$hook||'post.php'===$hook)
			if(in_array($post->post_type, $slider_options_keys)){
				...
				wp_enqueue_script('tesla-tinymce', tesla_locate_uri('tesla_framework/static/js/tinymce/tinymce.min.js'),array(),null);
	...
	private static function generate_input($type, $name, $value='', $placeholder='', $label='', $default='', $disabled=false){
		...
		switch($type){
			...
			case 'editor':
				$output .= '<textarea rows="1" cols="40" name="'.$name.'" placeholder="'.$placeholder.'" '.$disabled.' class="tesla-input-editor">'.$value.'</textarea>';
				break;
