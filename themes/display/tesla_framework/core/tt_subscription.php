    ...
    public static function insert_subscription_ajax(){
        ...
        if(!empty($data)){
            ...
            if ( fputcsv( $fh, $data ,"\t") &&                  //write txt file
                ...
                $result = (!empty(self::$config['success_msg'])) ? self::$config['success_msg'] : __('Subscribed',THEME_NAME);
            else
              $result = (!empty(self::$config['error_writing_msg']))?self::$config['error_writing_msg'] : __('Error',THEME_NAME);
            ...
            if(_go('mailchimp')){       //send api call to mailchimp if so selected in FW
                ...
                $result = $mailchimp_msg === true ? $result : (!empty($mailchimp_msg) ? $mailchimp_msg : (!empty(self::$config['error_mailchimp']) ? self::$config['error_mailchimp'] : __('MailChimp Error',THEME_NAME)));
            }
            ...
            $result = ( !empty( self::$config['no_data_posted_msg'] ) ) ? self::$config['no_data_posted_msg'] : __('No data received',THEME_NAME);
        
        //$response['result'] = $result;
        die(json_encode($result));
    }

    private function mailchimp_call($data,$grab_error = false){
        if(_go('mailchimp_api_key') && _go('mailchimp_list_id')){
            ...
            if(preg_match('@-(.*)@is',$apikey,$dc_matches)){
                $dc = $dc_matches[1];
                //$url = "https://$dc.api.mailchimp.com/2.0/lists/members?apikey=$apikey&id=5d900e3c31";  //gets members of a list with id
                //$url = "https://$dc.api.mailchimp.com/2.0/lists/list";    //gets all user's lists
            ...
            $result = ($grab_error) ? __('No API key for mailchimp',THEME_NAME) : FALSE;
        return $result;
    }
