...
class TT_ENQUEUE {
    ...
    public static function init_enqueue(){
        add_action( "wp_enqueue_scripts", array( 'TT_ENQUEUE', 'load_scripts' ) );
        add_action( "wp_enqueue_scripts", array( 'TT_ENQUEUE', 'load_stylesheet' ) );
        add_action( 'admin_enqueue_scripts', array( 'TT_ENQUEUE','admin_load_scripts' ) );
        add_action( 'admin_enqueue_scripts', array( 'TT_ENQUEUE','admin_load_stylesheet' ) );
        self::get_scripts();
    }
    ...
    private static function get_scripts(){
        self::reset_temp();
        $js_files = self::$js_files = self::scan_directory( TT_THEME_DIR . self::$js_dir , 'js');
        self::reset_temp();
        $css_files = self::$css_files = self::scan_directory( TT_THEME_DIR . self::$css_dir , 'css');
        ...
        foreach (self::$js_files as $js_file) {
            if(strpos($js_file, '/admin/') === FALSE)
                if(basename($js_file) != self::$main_js)
                    if(strpos($js_file, 'http') !== FALSE){     //if external link
