...
?>
<h2 class="my-account-title"><?php _e( 'Order Details', GETTEXT_DOMAIN ); ?></h2>
			...
			foreach($order->get_items() as $item) {
				...
				if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
					...
					foreach ( $download_file_urls as $file_url => $download_file_url ) {
						...
						$links[] = '<small><a href="' . $download_file_url . '">' . sprintf( __( 'Download file%s', GETTEXT_DOMAIN ), ( count( $download_file_urls ) > 1 ? ' ' . ( $i + 1 ) . ': ' : ': ' ) ) . $filename . '</a></small>';
...
<?php if ( get_option('woocommerce_allow_customers_to_reorder') == 'yes' && $order->status=='completed' ) : ?>
	<p class="order-again">
		<a href="<?php echo esc_url( $woocommerce->nonce_url( 'order_again', add_query_arg( 'order_again', $order->id, add_query_arg( 'order', $order->id, get_permalink( woocommerce_get_page_id( 'view_order' ) ) ) ) ) ); ?>" class="button"><?php _e( 'Order Again', GETTEXT_DOMAIN ); ?></a>
	</p>
<?php endif; ?>
...
<header>
	<h2 class="my-account-title"><?php _e( 'Customer details', GETTEXT_DOMAIN ); ?></h2>
</header>
...
<?php endif; ?>
		...
		<header class="title">
			<h3 class="my-account-title"><?php _e( 'Billing Address', GETTEXT_DOMAIN ); ?></h3>
		</header>
		<address><p>
			<?php
...
<?php if (get_option('woocommerce_ship_to_billing_address_only')=='no') : ?>
	...
	<div class="col-2">
		...
		<header class="title">
			<h3 class="my-account-title"><?php _e( 'Shipping Address', GETTEXT_DOMAIN ); ?></h3>
		</header>
		<address><p>
			<?php
