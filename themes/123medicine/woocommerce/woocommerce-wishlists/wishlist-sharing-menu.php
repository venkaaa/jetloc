...
if ($wishlist_items && count($wishlist_items)) :
    ...
    $twitter_message = $is_users_list ? __('Check out my wishlist at ', GETTEXT_DOMAIN) . get_bloginfo( 'name' ) . ' ' : __('Found an interesting list of products at ', GETTEXT_DOMAIN) . get_bloginfo( 'name' ) . ' ';
    ...
    <?php if (strstr($wishlist->get_wishlist_sharing(), 'Private') === false && ($e_email | $e_facebook | $e_twitter | $e_pinterest)) : ?>
        <ul class="wl-share-links">
            <li><?php _e('Share with Friends', GETTEXT_DOMAIN); ?></li>
            <?php if ($e_email) : ?>
                <li class="wl-email"><a class="wl-email-button" href="#share-via-email-<?php echo $wishlist->id; ?>"><?php _e('Email', GETTEXT_DOMAIN); ?></a></li>
            <?php endif; ?>
            <?php if ($e_facebook) : ?>
                <li class="wl-facebook"><a target="_blank" href="<?php echo $facebook_url; ?>"><?php _e('Facebook', GETTEXT_DOMAIN); ?></a></li>
            <?php endif; ?>
            <?php if ($e_twitter) : ?>
                <li class="wl-twitter"><a target="_blank" href="<?php echo $twitter_url; ?>"><?php _e('Twitter', GETTEXT_DOMAIN); ?></a></li>
            <?php endif; ?>
            <?php if ($e_pinterest && $pinterest_img_url) : ?>
                <li class="wl-pinterest">
                    <a href="http://pinterest.com/pin/create/button/?url=<?php $wishlist->the_url_view(true); ?>&media=<?php echo $pinterest_img_url; ?>&amp;description=<?php $wishlist->the_title(); ?>" class="hide-text" target="_blank">Pin It on Pinterest!</a>
                </li>
            <?php endif; ?>
        </ul>
    <?php endif; ?>
