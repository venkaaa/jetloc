...
<div id="order_review">
	...
	<table class="shop_table">
		...
		<tfoot>
			...
			<tr class="discount">
				<th><?php _e( 'Cart Discount', GETTEXT_DOMAIN ); ?></th>
			...
			<tr class="discount">
				<th><?php _e( 'Order Discount', GETTEXT_DOMAIN ); ?></th>
					...
					<?php
						...
						if ( $woocommerce->cart->tax_display_cart == 'incl' ) {
							...
							if ( ! empty( $tax_string_array ) ) {
								?><small class="includes_tax"><?php printf( __( '(Includes %s)', GETTEXT_DOMAIN ), implode( ', ', $tax_string_array ) ); ?></small><?php
							}
						}
					?>
				</td>
	...
	<div id="payment">
		...
		<div class="form-row place-order">
			...
			echo apply_filters('woocommerce_order_button_html', '<input type="submit" class="btn btn-primary" name="woocommerce_checkout_place_order" id="place_order" value="' . $order_button_text . '" />' );
			?>

			<?php if (woocommerce_get_page_id('terms')>0) : ?>
			<p class="form-row terms">
				<label for="terms" class="checkbox"><?php _e( 'I have read and accept the', GETTEXT_DOMAIN ); ?> <a href="<?php echo esc_url( get_permalink(woocommerce_get_page_id('terms')) ); ?>" target="_blank"><?php _e( 'terms &amp; conditions', GETTEXT_DOMAIN ); ?></a></label>
