	...
	<?php }?>
            ...
            <div class="box-caption <?php if(!has_post_thumbnail()&&!$post_header_image_thumbnail&&!$video){?>no-caret<?php }?>">
                ...
                <div class="box-caption-bottom">
                    ...
                    <div class="mb_social">
                    	<a class="social social-icon facebook" href="http://www.facebook.com/sharer/sharer.php?s=100&p[url]=<?php the_permalink();?>&p[images][0]=<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'medium' ); echo $image[0];?>&p[title]=<?php the_title();?>&p[summary]=<?php echo lpd_excerpt(25)?>"></a>
                    	<a class="social social-icon twitter" href="http://twitter.com/home?status=<?php echo lpd_excerpt_more(10)?>, <?php the_permalink();?>"></a>
                    	<a class="social social-icon google_plus" href="https://plus.google.com/share?url=<?php the_permalink();?>"></a>
                    	<a class="social social-icon pinterest" href="http://pinterest.com/pin/create/button/?url=<?php the_permalink();?>&media=<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'medium' ); echo $image[0];?>&description=<?php echo lpd_excerpt(25)?>"></a>
                    </div>
                </div> 
                <?php }?>   
            </div>
