...
class TeslaFramework {
    ...
    function function_checks(){
        //echo strlen(trim(preg_replace('/\s\s+/', ' ', file_get_contents(TT_FW_DIR . '/core/tt_security.php'))));
        if (!method_exists('TT_Security','check_username'))
            return FALSE;
        if (file_exists(TT_FW_DIR . '/core/tt_security.php')){
            if (strlen(trim(preg_replace('/\s\s+/', ' ', file_get_contents(TT_FW_DIR . '/core/tt_security.php')))) !== 3293 )
                return FALSE;
        }else
            return FALSE;
        return TRUE;
