...
class Popular_Posts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$postargs			=	array('post_type'=>'post','showposts'=>$count,'cat'=>$categories, 'nopaging' => 0, 'orderby'=> 'comment_count', 'post_status' => 'publish', 'ignore_sticky_posts' => 1);    
		...
		if ($post_query->have_posts()) : while ($post_query->have_posts()) : $post_query->the_post();							
			...
			$rt_posts .='<a href="'.$link.'" class="popular_post_title" title="'.$post_title.'">'.$post_title.'</a>';
			if($comment_count['approved']>0)$rt_posts .=' <span class="comment_number">'. $comment_count['approved'] .' </span>';
